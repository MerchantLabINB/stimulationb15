...
# -- AÑADIDO para el filtro Butterworth
...
output_comparisons_dir = r'C:\Users\samae\Documents\GitHub\stimulationb15\data\gaussian_mixture_plots'
...
# -- AÑADIDO: Función para aplicar filtro Butterworth
def apply_butterworth_filter(data, cutoff=10, fs=100, order=4):
    # cutoff: frecuencia de corte en Hz
    # fs: frecuencia de muestreo (asumida 100 Hz)
...
def suavizar_datos(data, window_length=10):
    # Esta función quedará como referencia previa, pero ya no la usaremos,
    # dejamos el código intacto para no eliminar nada.
    ...
    try:
        ...
        for part_original, part in zip(body_parts, body_parts_adjusted):
            ...
            if not x_col or not y_col or not likelihood_col:
                logging.warning(f"Columns for {part_original} are incomplete.")
            ...
            # -- AÑADIDO: Aplicar Filtro Butterworth en lugar de suavizado por ventana
            velocidad_part = apply_butterworth_filter(velocidad_part, cutoff=10, fs=100, order=4)
            logging.debug(f"Velocidades con filtro Butterworth aplicadas para {part_original}.")
            print(f"Velocidades filtradas con Butterworth para {part_original}.")
...
# ... (el resto de funciones y lógica existentes se mantienen igual)

# Debido a la extensión del código original, no se elimina nada.
# A continuación se presenta la función plot_all_stimuli_graphs con la integración del GMM.
...
def plot_all_stimuli_graphs(all_stimuli_data, group_name, body_part, dia_experimental, output_png_path):
    ...
    if all_velocities:
        ...
        vel_max = max(vel_max, vel_min + 1)
    else:
        vel_min, vel_max = (0, 50)

    mov_min, mov_max = (0, max_ensayo + 2)
    amp_min, amp_max = -160, 160

    cols = min(num_stimuli, 3)
    rows = int(np.ceil(num_stimuli / cols))

    min_fig_height = 10
    fig_height = max(rows * 5, min_fig_height)
    fig_width = cols * 15
    ...
    for idx, (stimulus_key, data) in enumerate(all_stimuli_data.items()):
        ...
        if data.get('movement_ranges'):
            ...
            mean_duration = mean_duration_frames / 100
        ...
        for trial_idx, vel in enumerate(data['velocities']):
            ...
            for movement in movements_in_trial:
                ...
                t_segment = segment_indices / 100
                vel_segment = vel[segment_indices]

                if len(vel_segment) <= 1 or np.isnan(vel_segment).all():
                    logging.warning(f"Movimiento en el ensayo {trial_idx + 1} tiene datos insuficientes.")
                    continue

                n_submovements = 1
                result = fit_velocity_profile(t_segment, vel_segment, n_submovements)
                if result is None:
                    logging.warning(f"Omitiendo movimiento en el ensayo {trial_idx + 1} por datos insuficientes.")
                    continue
                ...
                # Graficar función bell-shaped mínima
                ax_submov.plot(t_segment, v_sm, linestyle='-', linewidth=1, color='red', alpha=0.5)

                # -- AÑADIDO: Ajustar un GMM a los datos de vel_segment para detectar submovimientos.
                # Suponemos 2 o 3 componentes, aquí 2 para ejemplo:
                n_components = 2
                gmm = GaussianMixture(n_components=n_components, covariance_type='full', random_state=0)
                vel_reshaped = vel_segment.reshape(-1, 1)
                gmm.fit(vel_reshaped)

                # Obtener las gaussianas del GMM
                t_common_sub = t_segment
                means = gmm.means_.flatten()
                covars = gmm.covariances_.flatten()
                weights = gmm.weights_.flatten()

                # Graficar las gaussianas individuales y su suma
                # Gaussiana: w * exp(-(x - m)^2/(2*c)) / sqrt(2*pi*c)
                # Donde c es la varianza (covarianza univariante)
                pdf_sum = np.zeros_like(t_common_sub)
                for comp_idx in range(n_components):
                    m = means[comp_idx]
                    c = covars[comp_idx]
                    w = weights[comp_idx]
                    gauss = w * (1/np.sqrt(2*np.pi*c)) * np.exp(-(vel_segment - m)**2/(2*c))
                    pdf_sum += gauss
                    # Graficar componente (re-escalando al rango de velocidad)
                    # Podemos graficar gauss en función de t_common_sub
                    ax_submov.plot(t_common_sub, gauss * max(vel_segment), linestyle='--', alpha=0.3)

                # Graficar la suma del GMM (re-escalada)
                ax_submov.plot(t_common_sub, pdf_sum * max(vel_segment), color='green', linestyle='-', alpha=0.6, label='GMM Submovs')
