...
def collect_velocity_threshold_data():
    ...
    # Aquí seguimos almacenando todo
    movement_ranges_all = []
                                    ...
                                    if part in velocidades:
                                        ...
                                        if len(indices_above) > 0:
                                            ...
                                            for seg_idx, seg_above in enumerate(segments):
                                                ...
                                                # ============ THRESHOLD-BASED MOVEMENT STATS ================
                                                movement_data = {
                                                    ...
                                                    'Duración (ms)': duracion_ms,
                                                    'MovementType': 'Threshold-based'  # <--- NEW
                                                }
                                                ...
                                                if periodo == 'Durante Estímulo':
                                                    ...
                                                    if result is None:
                                                        ...
                                                        for i_sub in range(n_submov):
                                                            ...
                                                            # ================= NEW CODE FOR GAUSSIAN-BASED SUBMOVEMENT STATS ==================
                                                            if gauss_params:
                                                                ...
                                                                # The submovement "duration" is from mu-3*sigma to mu+3*sigma:
                                                                start_g  = (mu_gauss - 3*sig_gauss)
                                                                end_g    = (mu_gauss + 3*sig_gauss)
                                                                # Convert to frames if needed, or just store in seconds:
                                                                # Check for negative times, or times > total, etc.
                                                                if start_g < 0: start_g = 0
                                                                if end_g   > t_segment[-1]: end_g = t_segment[-1]
                                                                dur_g     = end_g - start_g

                                                                # Because we want the same 4 metrics:
                                                                gauss_submov_data = {
                                                                    'Ensayo': trial_counter+1,
                                                                    'body_part': part,
                                                                    'Dia experimental': dia_experimental,
                                                                    ...
                                                                    'Forma del Pulso': forma_pulso.capitalize(),
                                                                    'Duración (ms)': duracion_ms,
                                                                    'Periodo': 'Durante Estímulo',  # always inside
                                                                    'MovementType': 'Gaussian-based',

                                                                    # In frames -> to seconds:
                                                                    'Latencia al Inicio (s)': start_g - (start_frame/100.0),
                                                                    'Latencia al Pico (s)':   mu_gauss - (start_frame/100.0),
                                                                    'Valor Pico (velocidad)': A_gauss,
                                                                    'Duración Total (s)':     dur_g,
                                                                    'Duración durante Estímulo (s)': dur_g  # or partial overlap
                                                                }
    ...
    # Aquí llamamos a la función de boxplots
